...
HEADER = 'Pure Storage List FA Database Volumes (' + VERSION + ')'
BANNER = ('=' * 102)
DEBUG_LEVEL = 0
VERBOSE_FLAG = False
COOKIE = ''
...
def list_volumes(flashArray):
    # Create new file w=write, a=append, x=create
    f = open("log/dbVolumes.txt", "w")

    data = ''
    params = json.dumps(data)
    
    path = "/api/" + API_VERSION + "/volume?filter=key='dbName'&tags=true&namespace=Oracle"
    ...
    if res == 0:
        print('No Database Volumes found')
    else:
        x = 0
        print(BANNER)
        ...
        while (x<res):
            #
            volName = (jsonData[x]['name'])

            if VERBOSE_FLAG:
                print('Found Database Volume', volName)

            path = '/api/' + API_VERSION + '/volume/' + volName + '?space=true'

            # Perform action
            jsonData1 = get_url(flashArray,path,params)

            name = (jsonData1['name'])
            size = (jsonData1['size'])
            used = (jsonData1['volumes'])
...
def parsecl():
    ...
    parser.add_option('-w', '--wait_time',
                      type = 'int',
                      dest = 'wait_time',
                      default = 60,
                      help = 'Wait Time in minutes [default: %default]')
...
def main():
    ...
    wait_time = options.wait_time * 60 
    ...
    while True:
      ...
      print(result,'Wait ', wait_time/60,'minutes')
